...
| Staff Android app (.apk, sideloaded) | `android-app/` — native Kotlin WebView wrapper (`MainActivity.kt`, package `com.kcmps.app`) loading **live** kcmps.com, so site deploys never need an app rebuild. Current: **v1.1.0** — appends ` KCMPSApp/1.1` to the WebView User-Agent (both WebViews, via `createWebView()`), which is what activates the site's persistent sign-in (see the auth-tokens gotcha below), and sets `allowBackup=false`. NOT a TWA (Chrome's unsuppressible first-launch toast was rejected); Google sign-in doesn't work inside it, username/password does (login popup relays tokens via shared WebView storage). Keystore `android-app/signing/` is gitignored — **owner-held, back it up, same key forever**. Superseded APKs go to `android-app/previous-apk-versions/` (move + update its README version log, never delete — owner convention 2026-08-20). Build: `~/gradle/gradle-8.7/bin/gradle assembleRelease` (SDK `~/android-sdk`, JDK 21); test AVD `kcmps-test`. Gotcha: `restoreState()` with an empty history must fall back to `loadUrl` or the app is a permanent white screen (v1.0.0 bug). PWA extras (`website/manifest.webmanifest`, `website/assets/app-icons/`) are TWA leftovers, benign. See `docs/history.md` entries 74–75 |
...
- **Auth tokens** live in `sessionStorage`, not `localStorage` (deliberate XSS-exposure
  ...
  **One scoped exception (2026-08-20): inside the staff Android app** (detected via the
  ` KCMPSApp/` User-Agent token) `auth-refresh.js`'s `tokenStore` seam stores the same key in
  `localStorage` instead, backed by the dedicated 30-day app client
  (`kcmps-app-client`, `44m8ihv638uupblm8dscurgu7g`) — Messenger-style persistent sign-in,
  reset only by the Logout button. Every token read/write in the codebase routes through that
  seam; `IS_KCMPS_APP` is answered ONCE in `auth-refresh.js` and consumed via `KCMPS_AUTH`,
  never re-derived. Browsers keep the web client + sessionStorage byte-identically — every
  app-mode branch must stay additive and false outside the app. Both client ids are audiences
  on the staging AND production API JWT authorizers (prod: `sboj1n` on `6msg2uho6c`, CLI-managed
  — a client id missing there = login works, every API call 401s). See `docs/history.md` entry 75.
