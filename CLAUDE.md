...
| Design asset library plan | `docs/roadmap.md` "Parallel track — Design Asset Library" — private S3 + foundation table, category reuses catalog leaves, access reuses Production/Sales/Admin groups. Backend and dashboard UI are both live on staging (see the two rows above); `store.js`'s manifest-merge into `buildDesignGrid()` (consuming `design-manifest.json` on the storefront side) is the one piece still outstanding |
