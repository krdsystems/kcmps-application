...
| Design asset library plan | `docs/roadmap.md` "Parallel track — Design Asset Library" — private S3 + foundation table, category reuses catalog leaves, access reuses Production/Sales/Admin groups. Backend now fully built on staging (see the row above); only the dashboard UI (`website/dashboard/design-library.html`) and `store.js`'s manifest-merge into `buildDesignGrid()` remain |
