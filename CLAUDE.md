...
| Staff Android app (.apk, sideloaded) | `android-app/` — native Kotlin WebView wrapper (`MainActivity.kt`, package `com.kcmps.app`) loading **live** kcmps.com, so site deploys never need an app rebuild. NOT a TWA (Chrome's unsuppressible first-launch toast was rejected); Google sign-in doesn't work inside it, username/password does (login popup relays tokens via shared `localStorage`). Keystore `android-app/signing/` is gitignored — **owner-held, back it up, same key forever**. Build: `~/gradle/gradle-8.7/bin/gradle assembleRelease` (SDK `~/android-sdk`, JDK 21); test AVD `kcmps-test`. Gotcha: `restoreState()` with an empty history must fall back to `loadUrl` or the app is a permanent white screen (v1.0.0 bug). PWA extras (`website/manifest.webmanifest`, `website/assets/app-icons/`) are TWA leftovers, benign. See `docs/history.md` entry 74 |
