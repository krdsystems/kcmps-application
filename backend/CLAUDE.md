...
| `upload-design-file.js` | `POST /design-uploads` — presigned S3 PUT for a customer's own artwork/print file at checkout (the drag-and-drop zone next to store.js's `#co-notes`). No auth (guest checkout), no orderId yet (pre-checkout — the ref travels into `create-order.js`'s `designFiles`). Allowlist lives in `../lib/upload-types.js`; **SVG and archives are excluded on purpose**. Read its header before touching any of it — the validation layers, the forced-download rule, and the GuardDuty scanning decision (incl. its ~₱75/mo cost) are all documented there |
