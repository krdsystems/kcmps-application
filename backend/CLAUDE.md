...
| File | Purpose |
|---|---|
| `constants.js` | `SITE_ID`, `CURRENCY`, `SCHEMA_VERSION`, the canonical `STATUS` vocabulary, `ACTIVE_STATUSES`/`TERMINAL_STATUSES`, and `SCAN_STATUS`/`isCleanScanVerdict()` — the ONE fail-closed "is this GuardDuty verdict clean" predicate every upload read path (`staff-api/get-orders.js`, `staff-api/get-messages.js`, `checkout/lookup-order.js`) must use, added 2026-08-07 after a hand-rolled copy in `lookup-order.js` drifted and skipped the check entirely |
...
| `stations.js` | Production stations — `DEFAULT_STATIONS` (the no-migration fallback for the `CONFIG#STATIONS` item), `normalizeStations()`, `activeStations()`, `isKnownStation()`, `stationLabel()`, `validateStationsPayload()`. **Station ids are create-only**: they are already written onto line items and into append-only `EVENT#` records, so the validator refuses a payload that drops or renames one — retire instead. `isKnownStation()` accepts RETIRED ids on purpose, so a job already at a retired station stays advanceable. Unit-tested in `stations.test.js` |
...
| `lookup-order.js` | `POST /orders/lookup` — guest order lookup by `{orderId, contact}` (accepts either the full `ORD-xxxx` id or the bare suffix, case-insensitively — `../lib/keys.js`'s `normalizeOrderId()`), no authorizer. `contactsMatch()` (`../lib/customer-view.js`) is the real auth boundary since orderId alone isn't a meaningful secret; identical generic 404 for wrong-id vs wrong-contact, artificial delay, tight route throttle (5 req/s). The GCash screenshot presign is gated on `../lib/constants.js`'s `isCleanScanVerdict()` — the same fail-closed predicate `staff-api/get-orders.js` uses — after review found this Lambda's copy had drifted and skipped the scan-verdict check entirely (fixed 2026-08-07) |
...
| `stations.js` | `GET`/`PUT /stations` — the `CONFIG#STATIONS` config item. GET is `isStaff()` (every scheduler needs the picker); PUT is `requireRole([ADMIN])`. Writes an `EVENT#` audit record alongside the config so "who retired that station" is answerable later. Rules live in `../lib/stations.js` |
...
| `get-history.js` | `GET /cashbook/history` (optional `?month=`) — **Admin-only**, same gate as `get-month.js` since this is month totals served a year at a time. Reads only `METRIC#MONTH`/`METRIC#DAY` rollups (≤60 GetItems for a whole history), never transactions; year totals summed server-side so a year row can't disagree with its months |
| `search-transactions.js` | `GET /cashbook/search?q=` — `isStaff()`. Free-text across every day. **Not a Scan**: reads month rollups, skips months with no transactions, queries only the day partitions of active months. Uses parallel GetItems, not BatchGetItem — the shared staff-api role doesn't grant `dynamodb:BatchGetItem` |
